<template>
  ...
  <div v-else class="game">
    ...
    <div v-if="showWinner" class="message">
      <label v-if="!showResult" class="fail"> Fallaste!!</label>
      <label v-if="showResult" class="right"> Ganaste!!</label>
      <button @click="reiniciar()">Volver a intentarlo</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    comprobarClick(e) {
      ...
      if (this.winner.id === e) this.showResult = true;
    },
    reiniciar() {
      this.pokeArr = [];
      this.winner = null;
